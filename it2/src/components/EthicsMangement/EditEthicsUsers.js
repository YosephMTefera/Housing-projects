...
function EditEthicsUsers() {
  ...
  useEffect(() => {
    apiRequest
      .get(`/accusation_acceptor_user_api/get_accusation_acceptor/${id}`, {
        headers: {
  ...
  const editEthicsUser = async () => {
    try {
      ...
      setLoading(true);
      await apiRequest
        .put(`/accusation_acceptor_user_api/update_accusation_acceptor/${id}`, archivalData, {
          headers: {
            get_updaccus_api: process.env.REACT_APP_GET_UPDACCUS_API,
            Authorization: `Bearer ${token}`,
          },
        })
        .then((res) => {
  ...
  return (
    ...
    <div className="w-[90%] mt-[50px] mx-auto">
         ...
         {profileImg && typeof profileImg === "string" && (
            <div className="mx-auto my-[30px] flex flex-col gap-4">
              ...
              <div className="w-[100px] h-[100px]">
                <img
                  src={`${process.env.REACT_APP_BACKEND_IMAGES}/AccusationAcceptorImages/${profileImg}`}
                  alt="Current Pic"
      ...
      {loading ? <Loading addtionalStyle={"flex justify-end items-enter my-[20px]"}/> : 
...
export default EditEthicsUsers