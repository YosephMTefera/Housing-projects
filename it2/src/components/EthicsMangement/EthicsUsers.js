...
function EthicsUsers() {
  const navigate = useNavigate();
  const dispatch = useDispatch();
  const ethicsUsersState = useSelector((state) => state?.ethicsUsers);
  ...
  useEffect(() => {
    dispatch(fetchEthicsUsers({page:pageNum,sort:sortingNum,status,username:debouncedUsername,phone:debouncedPhone}));
  ...
  const handleNext = () => {
    if (pageNum >= ethicsUsersState?.ethicsUsers?.totalPages) {
      setPageNum(ethicsUsersState?.ethicsUsers?.totalPages);
    } else {
      setPageNum(pageNum + 1);
    }
  };

  if(ethicsUsersState.error) return <ServerError />
  return (
    <div className="w-[95%] mx-auto h-[82%] mt-[30px] relative bg-white rounded">
      <div className="w-[95%] mx-auto">
        <div className="flex items-center justify-between gap-[10px]  text-[#FBB042]">
          <div className="flex items-center justify-start">
            <BiChevronLeft
              ...
              onClick={() => navigate(-1)}
            />
            <span className="text-[20px] font-bold max-lg2:text-[16px]">Ethics Users</span>
          </div>

          <div>
            <button
              onClick={() => navigate("/createethicsuser")}
        ...
        {ethicsUsersState?.loading ? (
        <div></div>
      ) : (
        ethicsUsersState?.ethicsUsers?.accusationacceptors?.length > 0 &&
        ethicsUsersState?.ethicsUsers?.totalPages && (
          <div className="w-[100%] flex justify-end items-center my-[20px] gap-5">
            ...
            <span className="text-gray-600 font-semibold">
              {ethicsUsersState?.ethicsUsers?.currentPage} of{" "}
              {ethicsUsersState?.ethicsUsers?.totalPages}
            </span>
          ...
          {ethicsUsersState?.loading ? (
            ...
            <table className="w-[100%]">
              ...
              <tbody className="border [&>*:nth-child(even)]:bg-[#F9F9F9]">
                  ...
                  {ethicsUsersState?.ethicsUsers?.accusationacceptors?.map((au, index) => {
                      return (
                        <tr
                          key={index}
                          onClick={()=>navigate(`/ethicsusers/${au?._id}`)}
                        ...
                        >
                            ...
                            {au?.picture ? (
                              <div className="w-[50px] h-50px] bg-">
                                <img
                                  ...
                                  src={`${process.env.REACT_APP_BACKEND_IMAGES}/AccusationAcceptorImages/${au?.picture}`}
                                  alt=""
                                />
                              </div>
                            ) : (
                          ...
                          <td className="py-2 border text-[14px] max-lg2:text-[12px]">
                            <button
                              onClick={(e) =>{
                                e.stopPropagation();
                                navigate(`/editethicsuser/${au?._id}`);}
                              }
