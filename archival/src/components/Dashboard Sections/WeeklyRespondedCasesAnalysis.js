...
function WeeklyRespondedCasesAnalysis() {
    ...
    useEffect(() => {
        try {
          setLoading(true);
          apiRequest
            .get("/letter_analysis/weekly_responded_cases_analysis", {
              headers: {
  ...
  return (
    ...
    <div className='"w-[100%] col-span-1 border py-4 flex flex-col justify-center items-center rounded-[10px]'>
            ...
            {translationState?.lan ==="En" && language?.weeklyRespondedCases[0]}
            {translationState?.lan ==="Am" && language?.weeklyRespondedCases[1]}
            </span>
            ...
            {(weeklyLetterData?.ስኞ ===0 && weeklyLetterData?.ማክሰኞ ===0 && weeklyLetterData?.ዕሮብ ===0 && weeklyLetterData?.ሃሙስ ===0 && weeklyLetterData?.አርብ ===0) ?  <div className="w-[100%] mt-[20px]  min-h-[400px] bg-white p-4 rounded-[20px] flex justify-center items-center">
                <span className="font-bold text-[#0C73B8] max-lg2:text-[14px]">
                {translationState?.lan ==="En" && language?.noRespondedCasesWeek[0]}
                {translationState?.lan ==="Am" && language?.noRespondedCasesWeek[1]}
                </span>
...
export default WeeklyRespondedCasesAnalysis