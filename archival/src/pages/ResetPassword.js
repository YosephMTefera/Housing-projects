...
function ResetPassword() {
  ...
  const handleReset = async (e) => {
    ...
    try {
      ...
      await apiRequest
        .put(
          `/archival_user_api/reset_archival_user/${id}/${token}`,
          {
            password: password,
          },
          {
            headers: {
              get_resarch_api: process.env.REACT_APP_GET_RESARCH_API,
            },
          }
        )
        .then((res) => {
  ...
  return (
    <div className="w-[100%] flex flex-col  items-center h-[100vh] bg-gray-100">
      ...
      <div className="w-[30%] mt-[100px] shadow-md rounded-[10px] bg-white z-[10] p-4 flex flex-col justify-center items-center gap-[30px] max-lg2:mt-[50px] max-lg2:w-[50%] max-lg1:w-[70%] max-sm1:w-[90%]">
        ...
        <div className="w-[80%] mx-auto flex flex-col gap-[20px]">
          <div className="flex flex-col gap-[10px]">
            {/* <span className="text-[14px] font-bold text-[#0C73B8]">Password*  {emptypass && <span className='text-red-700 text-[12px]'>Password can not be empty</span>}</span> */}
