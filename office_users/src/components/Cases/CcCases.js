...
import { fetchCcCases } from "../../REDUX/slices/ccCasesSlice";
...
function CcCases() {
    ...
    const caseList = useSelector((state) => state?.ccCases);
    ...
    useEffect(() => {
        dispatch(fetchCcCases({page:pageNum,sort:sortingNum,caseNumber:debouncedCaseNumber,status,division,late,type}));
        // eslint-disable-next-line react-hooks/exhaustive-deps
      }, [pageNum,sortingNum,debouncedCaseNumber,status,division,late,type]);
  ...
  return (
    ...
    <div className="w-[90%] my-[30px] mx-auto">
      ...
      <div className="my-[20px]">
        <div className="w-[100%] grid grid-cols-3 gap-[10px]">
          ...
          <div className="w-[100%] col-span-1 border border-gray-400 p-3 rounded-[5px]">
            <select onChange={(e)=>setType(e.target.value)} className="w-[100%] flex-1 bg-transparent outline-none text-[14px] max-lg2:text-[12px]">
              <option value={""}>
              {translationState?.lan ==="En" && language?.cc[0]}
              {translationState?.lan ==="Am" && language?.cc[1]}
              </option>
              <option value={"forward"}>
              {translationState?.lan ==="En" && language?.forwardCc[0]}
              {translationState?.lan ==="Am" && language?.forwardCc[1]}
              </option>
              <option value={"reply"}>
              {translationState?.lan ==="En" && language?.replyCc[0]}
              {translationState?.lan ==="Am" && language?.replyCc[1]}
              </option>
    ...
    {translationState?.lan ==="Am" && language?.pending[1]}
      ...
      ) : caseList?.cCCases?.cases?.length === 0 ? (
        <div className="flex justify-center items-center my-[30px]">
          <span className="text-[#0C73B8] text-[14px] font-bold">
          {translationState?.lan ==="En" && language?.noccCases[0]}
          {translationState?.lan ==="Am" && language?.noccCases[1]}
          </span>
        </div>
      ) : (
              ...
              <div className="shadow overflow-hidden border-b border-gray-200 sm:rounded-lg">
                <table className="min-w-full divide-y divide-gray-200">
                  ...
                  <tbody className="bg-white divide-y divide-gray-200">
                    {caseList?.cCCases?.cases?.map((case1, index) => {

                      return (
                        <tr
                          key={index}
                          className="text-center text-[12px] cursor-pointer"
                          onClick={() => navigate(`/cases/${case1?._id}/cc`)}
                        >
