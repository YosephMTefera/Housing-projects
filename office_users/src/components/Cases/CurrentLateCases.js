...
function CurrentLateCases() {
    ...
    const [lateCaseList,setLateCaseList] = useState({})
    ...
    useEffect(()=>{
       try {
        setLoading(true)
        apiRequest.get(`/customer_case_api/get_late_case?page=${pageNum}&sort=${sortingNum}&status=${status}&division=${division}&case_number=${debouncedCaseNumber}`,{headers:{
            get_ltecanalysis_api:process.env.REACT_APP_GET_LTECANALYSIS_API,
            Authorization:`Bearer ${token}`
          }}).then((res)=>{
            setLoading(false);
          
            setLateCaseList(res.data);
          }).catch((error)=>{
            setLoading(false);
      ...
      },[token,debouncedCaseNumber,division,pageNum,sortingNum,status])
  ...
  const handleNext = () => {
    if (pageNum >= lateCaseList?.totalPages) {
      setPageNum(lateCaseList?.totalPages);
    } else {
      setPageNum(pageNum + 1);
    }
  };
    ...
    <div className="w-[90%] my-[30px] mx-auto">
      ...
      <div className="my-[20px] grid grid-cols-3 gap-[10px]">
          ...
          <div className="w-[100%] border border-gray-400 p-3 rounded-[5px]">
            <select onChange={(e)=>setDivision(e.target.value)} className="w-[100%] flex-1 bg-transparent outline-none text-[14px]">
              <option value={""}>
      ...
      {loading ? (
            <div></div>
              ...
              lateCaseList?.cases?.length > 0 &&
              lateCaseList?.totalPages && (
              ...
              <div className="shadow overflow-hidden border-b border-gray-200 sm:rounded-lg">
                <table className="min-w-full divide-y divide-gray-200">
                  ...
                  <tbody className="bg-white divide-y divide-gray-200">
                    {lateCaseList?.cases?.map((case1, index) => {
                      return (
                        <tr
                          key={index}
                        ...
                        >
                          ...
                          <td className="px-6 py-4 whitespace-nowrap">
                            <div className="flex justify-center items-center">
                                ...
                                {case1?.customer_id &&  <div className="px-6 py-2 whitespace-nowrap text-sm text-gray-500">
                                  {case1?.customer_id?.firstname}{" "}
                                  {case1?.customer_id?.middlename}{" "}
                                  {case1?.customer_id?.lastname}
                                </div>}

                                {case1?.customer_info &&  <div className="px-6 py-2 whitespace-nowrap text-sm text-gray-500">
...
export default CurrentLateCases