...
import { fetchreplyoutgoingLetter } from '../../../REDUX/slices/replyOutgoingLetterSlice';
import apiRequest from '../../../utils/request';

function RepliedOutgoingLetter() {
  ...
  const outgoingLetterState = useSelector((state)=>state?.replyOutgoingLetters);
  ...
  useEffect(()=>{
    dispatch(fetchreplyoutgoingLetter({page:pageNum,sort:sortingNum,outgoingLtrNum:debouncedOutgoingLetterNum,status,late,type}))
  ...
  useEffect(() => {
    const fetchReplyPaths = async () => {
      ...
      try {
        ...
        for (let outLetter of outgoingLetterState?.replyoutgoingLetters?.outgoingLetters) {
          const response = await  apiRequest
          .get(`/rply_outgoing_ltr_api/get_reply_outg_ltr_path/${outLetter?._id}`, {
            headers: {
              Authorization: "Token " + token,
             get_rpldoutptltrpath_api: process.env.REACT_APP_GET_RPLDOUTPTLTRPATH_API,
            },
          })
          replyPathsData[outLetter?._id] = response?.data?.repliedDocs;
        }
        setLoading(false);
  ...
  const handleNext = () => {
    if (pageNum >= outgoingLetterState?.replyoutgoingLetters?.totalPages) {
      setPageNum(outgoingLetterState?.replyoutgoingLetters?.totalPages);
    } else {
      setPageNum(pageNum + 1);
    }
  };
    ...
    <div className="w-[95%] my-[30px] mx-auto">
      <div className="flex pb-4 justify-between items-center border-b">
        <span className="text-[#0C73B8] font-bold text-[20px] max-lg2:text-[16px]">        
          {translationState?.lan==="En" && `${language?.repliedLetters[0]} (${language?.outgoing[0]})`}
        {translationState?.lan==="Am" && `${language?.repliedLetters[1]} (${language?.outgoing[1]})`}</span>
  ...
  {outgoingLetterState?.loading ? (
      ...
      outgoingLetterState?.replyoutgoingLetters?.outgoingLetters?.length > 0 &&
      outgoingLetterState?.replyoutgoingLetters?.totalPages && (
        <div className="w-[100%] flex justify-end items-center my-[20px] gap-5">
          ...
          <span className="text-gray-600 font-semibold">
            {outgoingLetterState?.replyoutgoingLetters?.currentPage} of{" "}
            {outgoingLetterState?.replyoutgoingLetters?.totalPages}
          </span>
...
{(loading || outgoingLetterState?.loading) ? <Loading addtionalStyle={"flex justify-center items-center"}/>:outgoingLetterState?.replyoutgoingLetters?.outgoingLetters?.length ===0 ?   <div className="flex justify-center items-center my-[30px]">
   ...
   <th
                ...
                <tbody className="bg-white divide-y divide-gray-200 border [&>*:nth-child(even)]:bg-[#F9F9F9]">
                  {outgoingLetterState?.replyoutgoingLetters?.outgoingLetters?.map((ol,index)=>{
                                         const outLetterReplyPath = repliedPath[ol?._id];
                                         const officeForwardedPath = outLetterReplyPath?.some((ou)=>ou?.from_office_user?._id ===userID);

                    return <tr
                      key={index}
                    className="text-center border border-gray-300 text-[12px] cursor-pointer max-lg2:text-[10px]"
                    onClick={() => navigate(`/letters/outgoing/replied/${ol?._id}`)}
                  >
...
export default RepliedOutgoingLetter