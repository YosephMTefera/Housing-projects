...
function TotalOutgoingLetters() {
  ...
  useEffect(()=>{
    dispatch(fetchOutgoingLetter({page:pageNum,sort:sortingNum,outgoingLtrNum:debouncedOutgoingLetterNum,late,createdBy:"",status}))
    ...
    <div className="w-[95%] my-[30px] mx-auto">
      ...
      <IoChevronBack
        ...
        <span className="text-[#0C73B8] font-bold text-[20px] max-lg2:text-[16px]">        
          {translationState?.lan==="En" && `${language?.totalOutgoingLetters[0]}`}
        {translationState?.lan==="Am" && `${language?.totalOutgoingLetters[1]}`}</span>
   ...
   <th
                ...
                <tbody className="bg-white divide-y divide-gray-200 border [&>*:nth-child(even)]:bg-[#F9F9F9]">
                  {outgoingLetterState?.outgoingLetters?.outgoingLetters?.map((ol,index)=>{
                    ...
                    onClick={() => navigate(`/letters/outgoing/cc/${ol?._id}`)}
                  >
...
export default TotalOutgoingLetters