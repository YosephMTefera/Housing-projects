...
import { fetchForwardOutgoingLetter } from '../../../REDUX/slices/forwardOutgoingLetterSlice';
import apiRequest from '../../../utils/request';

function ForwardedOutgoingLetters() {
  ...
  const outgoingLetterState = useSelector((state)=>state?.forwardOutgoingLetters);
  ...
  useEffect(()=>{
    dispatch(fetchForwardOutgoingLetter({page:pageNum,sort:sortingNum,outgoingLtrNum:debouncedOutgoingLetterNum,status,late,type}))

  // eslint-disable-next-line react-hooks/exhaustive-deps
  },[pageNum,sortingNum,debouncedOutgoingLetterNum,late,status,type])
  ...
  useEffect(() => {
    const fetchForwardPaths = async () => {
      ...
      try {
        ...
        for (let outLetter of outgoingLetterState?.forwardOutgoingLetters?.outgoingLetters) {
          const response = await  apiRequest
          .get(`/frwd_outgoing_ltr_api/get_forward_outg_letter_path/${outLetter?._id}`, {
            headers: {
              Authorization: "Token " + token,
              get_frwdoutltrpath_api: process.env.REACT_APP_GET_FRWDOUTLTRPATH_API,
            },
          })
          forwardPathsData[outLetter?._id] = response?.data?.forwardDocs;
        }
        setLoading(false);
  ...
  const handleNext = () => {
    if (pageNum >= outgoingLetterState?.forwardOutgoingLetters?.totalPages) {
      setPageNum(outgoingLetterState?.forwardOutgoingLetters?.totalPages);
    } else {
      setPageNum(pageNum + 1);
    }
  };
    ...
    <div className="w-[95%] my-[30px] mx-auto">
      <div className="flex pb-4 justify-between items-center border-b">
        <span className="text-[#0C73B8] font-bold text-[20px] max-lg2:text-[16px]">        
          {translationState?.lan==="En" && `${language?.forwardedLetters[0]} (${language?.outgoing[0]})`}
        {translationState?.lan==="Am" && `${language?.forwardedLetters[1]} (${language?.outgoing[1]})`}</span>
  ...
  {outgoingLetterState?.loading ? (
      ...
      outgoingLetterState?.forwardOutgoingLetters?.outgoingLetters?.length > 0 &&
      outgoingLetterState?.forwardOutgoingLetters?.totalPages && (
        <div className="w-[100%] flex justify-end items-center my-[20px] gap-5">
          ...
          <span className="text-gray-600 font-semibold">
            {outgoingLetterState?.forwardOutgoingLetters?.currentPage} of{" "}
            {outgoingLetterState?.forwardOutgoingLetters?.totalPages}
          </span>
...
{(loading || outgoingLetterState?.loading) ? <Loading addtionalStyle={"flex justify-center items-center"}/>:outgoingLetterState?.forwardOutgoingLetters?.outgoingLetters?.length ===0 ?   <div className="flex justify-center items-center my-[30px]">
   ...
   <th
                ...
                <tbody className="bg-white divide-y divide-gray-200 border [&>*:nth-child(even)]:bg-[#F9F9F9]">
                  {outgoingLetterState?.forwardOutgoingLetters?.outgoingLetters?.map((ol,index)=>{
                        const outLetterForwardPath = forwardPath[ol?._id];
                        const officeForwardedPath = outLetterForwardPath?.some((ou)=>ou?.from_office_user?._id ===userID);

                    return <tr
                      key={index}
                    className="text-center border border-gray-300 text-[12px] cursor-pointer max-lg2:text-[10px]"
                    onClick={() => navigate(`/letters/outgoing/forwarded/${ol?._id}`)}
                  >
                ...
                font-semibold  bg-red-600 text-white rounded-[5px]"
                    ...
                    <td className="px-6 py-2 whitespace-nowrap border">
                      <div className="px-6 py-2 whitespace-nowrap  text-gray-500">
                       {ol?.createdBy?.firstname}    {ol?.createdBy?.middlename}    {ol?.createdBy?.lastname}
                      </div>
                    </td>
...
export default ForwardedOutgoingLetters