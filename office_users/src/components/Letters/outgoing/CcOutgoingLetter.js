...
import { fetchCcoutgoingLetter } from '../../../REDUX/slices/ccOutgoingLetterSlice';
import ServerError from '../../ServerError';

function CcOutgoingLetter() {
  const navigate = useNavigate();
  const dispatch = useDispatch();
  const translationState = useSelector((state)=>state.translation);
  const outgoingLetterState = useSelector((state)=>state?.ccOutgoingLetters);
  const [outgoingLtrNum,setoutgoingLtrNum]  = useState("");
  const [debouncedOutgoingLetterNum] = useDebounce(outgoingLtrNum,500)
  ...
  useEffect(()=>{
    dispatch(fetchCcoutgoingLetter({page:pageNum,sort:sortingNum,outgoingLtrNum:debouncedOutgoingLetterNum,status,late,type}))
  ...
  const handleNext = () => {
    if (pageNum >= outgoingLetterState?.ccoutgoingLetters?.totalPages) {
      setPageNum(outgoingLetterState?.ccoutgoingLetters?.totalPages);
    } else {
      setPageNum(pageNum + 1);
    }
  };
    ...
    <div className="w-[95%] my-[30px] mx-auto">
      <div className="flex pb-4 justify-between items-center border-b">
        <span className="text-[#0C73B8] font-bold text-[20px] max-lg2:text-[16px]">        
          {translationState?.lan==="En" && `${language?.ccLetters[0]} (${language?.outgoing[0]})`}
        {translationState?.lan==="Am" && `${language?.ccLetters[1]} (${language?.outgoing[1]})`}</span>
      ...
      <div className="my-[20px] grid grid-cols-4 gap-[10px]">
        <div className="col-span-1 border border-gray-300 p-3 h-[90%] flex items-center gap-[10px] rounded-[5px]">
          ...
          {translationState?.lan==="En" &&  <input
            ...
            onChange={(e)=>setoutgoingLtrNum(e.target.value)}
  ...
  {outgoingLetterState?.loading ? (
      ...
      outgoingLetterState?.ccoutgoingLetters?.outgoingLetters?.length > 0 &&
      outgoingLetterState?.ccoutgoingLetters?.totalPages && (
        <div className="w-[100%] flex justify-end items-center my-[20px] gap-5">
          ...
          <span className="text-gray-600 font-semibold">
            {outgoingLetterState?.ccoutgoingLetters?.currentPage} of{" "}
            {outgoingLetterState?.ccoutgoingLetters?.totalPages}
          </span>
...
{outgoingLetterState?.loading ? <Loading addtionalStyle={"flex justify-center items-center"}/>:outgoingLetterState?.ccoutgoingLetters?.outgoingLetters?.length ===0 ?   <div className="flex justify-center items-center my-[30px]">
            ...
            <div className="shadow overflow-hidden border-b border-gray-200 sm:rounded-lg">
              <table className="min-w-full divide-y divide-gray-200">
                ...
                <tbody className="bg-white divide-y divide-gray-200 border [&>*:nth-child(even)]:bg-[#F9F9F9]">
                  {outgoingLetterState?.ccoutgoingLetters?.outgoingLetters?.map((il,index)=>{

                    return <tr
                      key={index}
                    className="text-center border border-gray-300 text-[12px] cursor-pointer max-lg2:text-[10px]"
                    onClick={() => navigate(`/letters/outgoing/cc/${il?._id}`)}
                  >
                     ...
                     <td className="px-6 py-2 whitespace-nowrap border">
                      <div className="flex justify-center items-center">
                          ...
                          <div className="px-6 py-2 whitespace-nowrap  text-gray-500">
                           {il?.outgoing_letter_number? il?.outgoing_letter_number:"Not Assigned"}
                          </div>
                        </div>
                ...
                font-semibold  bg-red-600 text-white rounded-[5px]"
                    ...
                    <td className="px-6 py-2 whitespace-nowrap border">
                      <div className="px-6 py-2 whitespace-nowrap  text-gray-500">
                      {il?.createdAt?.split("T")[0]}
                      </div>
                    </td>

                    <td className="px-6 py-2 whitespace-nowrap  text-gray-500 border">
          
                    {il?.output_date?.split("T")[0]}
                    </td>
                    <td className="px-6 py-2 whitespace-nowrap  text-gray-500 border">
          
          {il?.verified_date?.split("T")[0]}
          </td>
...
export default CcOutgoingLetter