...
import { fetchreplyIncomingLetter } from '../../../REDUX/slices/replyIncomingLettersSlice';
import ServerError from '../../ServerError';
import apiRequest from '../../../utils/request';
function RepliedIncomingLetter() {
  ...
  const incomingLetterState = useSelector((state)=>state?.replyIncomingLetters);
  ...
  useEffect(()=>{
    dispatch(fetchreplyIncomingLetter({page:pageNum,sort:sortingNum,incomingLtrNum:debouncedIncomingLetterNum,attentionFrom:debouncedFrom,late}))
  ...
  useEffect(() => {
    const fetchReplyPaths = async () => {
      ...
      try {
        ...
        for (let incLetter of incomingLetterState?.replyIncomingLetters?.incomingLetters) {
          const response = await  apiRequest
          .get(`/reply_incoming_ltr_api/get_reply_inc_ltr_path/${incLetter?._id}`, {
            headers: {
              Authorization: "bearer " + token,
              get_rplyincltrpath_api: process.env.REACT_APP_GET_RPLYINCLTRPATH_API,
            },
          })
          replyPathsData[incLetter?._id] = response?.data?.repliedDocs;
        }
        setLoading(false);
  ...
  const handleNext = () => {
    if (pageNum >= incomingLetterState?.replyIncomingLetters?.totalPages) {
      setPageNum(incomingLetterState?.replyIncomingLetters?.totalPages);
    } else {
      setPageNum(pageNum + 1);
    }
  };
    ...
    <div className="w-[95%] my-[30px] mx-auto">
      <div className="flex pb-4 justify-between items-center border-b">
        <span className="text-[#0C73B8] font-bold text-[20px] max-lg2:text-[16px]">        
          {translationState?.lan==="En" && `${language?.repliedLetters[0]} (${language?.incoming[0]})`}
        {translationState?.lan==="Am" && `${language?.repliedLetters[1]} (${language?.incoming[1]})`}</span>
  ...
  {incomingLetterState?.loading ? (
      ...
      incomingLetterState?.replyIncomingLetters?.incomingLetters?.length > 0 &&
      incomingLetterState?.replyIncomingLetters?.totalPages && (
        <div className="w-[100%] flex justify-end items-center my-[20px] gap-5">
          ...
          <span className="text-gray-600 font-semibold">
            {incomingLetterState?.replyIncomingLetters?.currentPage} of{" "}
            {incomingLetterState?.replyIncomingLetters?.totalPages}
          </span>
...
{(loading || incomingLetterState?.loading) ? <Loading addtionalStyle={"flex justify-center items-center"}/>:incomingLetterState?.replyIncomingLetters?.incomingLetters?.length ===0 ?   <div className="flex justify-center items-center my-[30px]">
            ...
            <div className="shadow overflow-hidden border-b border-gray-200 sm:rounded-lg">
              <table className="min-w-full divide-y divide-gray-200">
                ...
                <tbody className="bg-white divide-y divide-gray-200 border [&>*:nth-child(even)]:bg-[#F9F9F9]">
                  {incomingLetterState?.replyIncomingLetters?.incomingLetters?.map((il,index)=>{
                     const incLetterReplyPath = repliedPath[il?._id];
                     const officeForwardedPath = incLetterReplyPath?.some((ou)=>ou?.from_office_user?._id ===userID);

                    return <tr
                      key={index}
                    className="text-center border border-gray-300 text-[12px] cursor-pointer max-lg2:text-[10px]"
                    onClick={() => navigate(`/letters/incoming/replied/${il?._id}`)}
                  >
...
export default RepliedIncomingLetter