...
import { fetchCcIncomingLetter } from '../../../REDUX/slices/ccIncomingLetterSlice';
import ServerError from '../../ServerError';

function CcIncomingLetter() {
  const navigate = useNavigate();
  const dispatch = useDispatch();
  const translationState = useSelector((state)=>state.translation);
  const incomingLetterState = useSelector((state)=>state?.ccIncomingLetters);
  ...
  useEffect(()=>{
    dispatch(fetchCcIncomingLetter({page:pageNum,sort:sortingNum,incomingLtrNum:debouncedIncomingLetterNum,attentionFrom:debouncedFrom,type}))

  // eslint-disable-next-line react-hooks/exhaustive-deps
  },[pageNum,sortingNum,debouncedIncomingLetterNum,debouncedFrom,type])
  ...
  const handleNext = () => {
    if (pageNum >= incomingLetterState?.ccIncomingLetters?.totalPages) {
      setPageNum(incomingLetterState?.ccIncomingLetters?.totalPages);
    } else {
      setPageNum(pageNum + 1);
    }
  };
    ...
    <div className="w-[95%] my-[30px] mx-auto">
      <div className="flex pb-4 justify-between items-center border-b">
        <span className="text-[#0C73B8] font-bold text-[20px] max-lg2:text-[16px]">        
          {translationState?.lan==="En" && `${language?.ccLetters[0]} (${language?.incoming[0]})`}
        {translationState?.lan==="Am" && `${language?.ccLetters[1]} (${language?.incoming[1]})`}</span>
  ...
  {incomingLetterState?.loading ? (
      ...
      incomingLetterState?.ccIncomingLetters?.incomingLetters?.length > 0 &&
      incomingLetterState?.ccIncomingLetters?.totalPages && (
        <div className="w-[100%] flex justify-end items-center my-[20px] gap-5">
          ...
          <span className="text-gray-600 font-semibold">
            {incomingLetterState?.ccIncomingLetters?.currentPage} of{" "}
            {incomingLetterState?.ccIncomingLetters?.totalPages}
          </span>
...
{incomingLetterState?.loading ? <Loading addtionalStyle={"flex justify-center items-center"}/>:incomingLetterState?.ccIncomingLetters?.incomingLetters?.length ===0 ?   <div className="flex justify-center items-center my-[30px]">
            ...
            <div className="shadow overflow-hidden border-b border-gray-200 sm:rounded-lg">
              <table className="min-w-full divide-y divide-gray-200">
                ...
                <tbody className="bg-white divide-y divide-gray-200 border [&>*:nth-child(even)]:bg-[#F9F9F9]">
                  {incomingLetterState?.ccIncomingLetters?.incomingLetters?.map((il,index)=>{

                    return <tr
                      key={index}
                    className="text-center border border-gray-300 text-[12px] cursor-pointer max-lg2:text-[10px]"
                    onClick={() => navigate(`/letters/incoming/cc/${il?._id}`)}
                  >
...
export default CcIncomingLetter