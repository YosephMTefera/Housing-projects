...
function CreatedInternalLetter() {
  ...
  useEffect(()=>{
    dispatch(fetchInternalLetter({page:pageNum,sort:sortingNum,internalLtrNum:debouncedInternalLetterNum,late,createdBy:userID,status}))
    ...
    <div className="w-[95%] my-[30px] mx-auto">
      <div className="flex pb-4 justify-between items-center border-b">
        <span className="text-[#0C73B8] font-bold text-[20px] max-lg2:text-[16px]">        
          {translationState?.lan==="En" && `${language?.createdLetters[0]} (${language?.internal[0]})`}
        {translationState?.lan==="Am" && `${language?.createdLetters[1]} (${language?.internal[1]})`}</span>
   ...
   <th
                ...
                <tbody className="bg-white divide-y divide-gray-200 border [&>*:nth-child(even)]:bg-[#F9F9F9]">
                  {internalLetterState?.internalLetters?.internalLetters?.map((il,index)=>{
                    ...
                    onClick={() => navigate(`/letters/internal/created/${il?._id}`)}
                  >
...
export default CreatedInternalLetter