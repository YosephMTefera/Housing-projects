...
import { fetchRepliedLetters } from "../../REDUX/slices/repliedLetterSlice";

function RepliedLetters() {
    const navigate = useNavigate();
    const dispatch = useDispatch();
    const letterList = useSelector((state) => state.repliedLetters);
  
    useEffect(() => {
      dispatch(fetchRepliedLetters());
    ...
    <div className="w-[90%] my-[30px] mx-auto">
      <div className="w-[100%] flex items-center justify-start   text-[#0C73B8] mt-[50px] mx-auto font-bold">
        <BiChevronLeft
          ...
          onClick={() => navigate(-1)}
        />
        <span className="text-[20px]">Replied Letters</span>
      </div>
      ...
      {letterList?.loading ? (
        ...
        letterList?.repliedLetters?.length ===0 ?  <div className="flex justify-center items-center my-[30px]">
        <span className="text-[#0C73B8] text-[14px]">You have no replied letters</span>
      </div>   :
              ...
              <div className="shadow overflow-hidden border-b border-gray-200 sm:rounded-lg">
                <table className="min-w-full divide-y divide-gray-200">
                  ...
                  <tbody className="bg-white divide-y divide-gray-200">
                    {letterList?.repliedLetters?.map((letter, index) => {
                      return (
                        <tr
                          key={index}
