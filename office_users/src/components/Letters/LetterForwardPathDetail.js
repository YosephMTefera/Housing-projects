...
function LetterForwardPathDetail() {
  const { id, type, forward_id } = useParams();
  ...
  useEffect(() => {
    if (type === "incoming") {
      ...
      apiRequest
        .get(`/forward_incoming_ltr_api/get_forward_inc_letter_path/${id}`, {
          headers: {
            ...
            get_frwdpath_incltr_api:
              process.env.REACT_APP_GET_FRWDPATH_INCLTR_API,
          },
        })
        .then((res) => {
          setLoading(false);
      ...
      toast.error(
        "Please specify the type of incoming, outgoing, internal or memo for which you want to check the forwarded path"
      );
    }
        ...
        <div className="w-[95%] mx-auto flex flex-col gap-5">
          <div className="flex justify-between items-center">
                ...
                {type === "incoming" && (
                  <>
                    {translationState?.lan === "En" &&
                      language?.forwardPathDetailIncoming[0]}
                    {translationState?.lan === "Am" &&
                      language?.forwardPathDetailIncoming[1]}
                  </>
                )}
                {type === "outgoing" && (
                  <>
                    {translationState?.lan === "En" &&
                      language?.forwardPathDetailOutgoing[0]}
                    {translationState?.lan === "Am" &&
                      language?.forwardPathDetailOutgoing[1]}
                  </>
                )}
                {type === "internal" && (
                  <>
                    {translationState?.lan === "En" &&
                      language?.forwardPathDetailInternal[0]}
                    {translationState?.lan === "Am" &&
                      language?.forwardPathDetailInternal[1]}
                  </>
                )}
                    {type === "presystem" && (
                  <>
                    {translationState?.lan === "En" &&
                      language?.forwardPathDetailPresystem[0]}
                    {translationState?.lan === "Am" &&
                      language?.forwardPathDetailPresystem[1]}
                  </>
                )}
                  {type === "memo" && (
                  <>
                    {translationState?.lan === "En" &&
                      language?.forwardPathDetailMemo[0]}
                    {translationState?.lan === "Am" &&
                      language?.forwardPathDetailMemo[1]}
                  </>
                )}
              </span>
            </div>
            <button
              onClick={() =>
                navigate(
                  `/letters/print/forward/${type}/${forwardPath?.forwardId}/${forwardPath?.forwardDocs[forward_id]?._id}`
                )
              }
            ...
            <div className="w-[100%] my-[40px] flex flex-col gap-[20px]">
              <div className="w-[100%] flex justify-between items-center gap-[50px]">
                <div className="flex flex-col gap-[20px]">
                  {forwardPath?.forwardDocs?.[forward_id]
                    ?.from_achival_user && (
                  ...
                  {forwardPath?.forwardDocs?.[forward_id]?.from_office_user && (
                  ...
                  <span className="text-gray-500 flex  items-center gap-[10px]">
                    ...
                    <span className="max-lg2:text-[14px]">
                      {forwardPath?.forwardDocs?.[forward_id]?.to?.firstname +
                        " " +
                        forwardPath?.forwardDocs?.[forward_id]?.to?.middlename +
                        " " +
                        forwardPath?.forwardDocs?.[forward_id]?.to?.lastname +
                        " - " +
                        forwardPath?.forwardDocs?.[forward_id]?.to?.position}
                    </span>
                  </span>
                  <span className="text-gray-500 flex  items-center gap-[10px]">
                    <span className="font-bold text-[#0C73B8] max-lg2:text-[14px]">
                    ...
                    <span className="max-lg2:text-[14px]">
                      {forwardPath?.forwardDocs?.[forward_id]?.cc}
                    </span>
                  </span>
                </div>
                <div className="flex  flex-col gap-[20px]">
                  <span className="text-gray-500 flex items-center gap-[10px]">
                    ...
                    <span className="max-lg2:text-[14px]">
                      {new Date(
                        forwardPath?.forwardDocs?.[forward_id]?.forwarded_date
                      ).toDateString()}
                    </span>
                  </span>
                  <span className="text-gray-500 flex items-center gap-[10px]">
          ...
          {forwardPath?.forwardDocs?.[forward_id]?.remark && (
...
export default LetterForwardPathDetail;
