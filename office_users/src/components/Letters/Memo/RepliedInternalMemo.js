...
import { fetchreplyinternalMemo } from '../../../REDUX/slices/replyInternalMemoSlice';
import apiRequest from '../../../utils/request';
function RepliedInternalMemo() {
  ...
  const internalMemoState = useSelector((state)=>state?.replyInternalMemo);
  ...
  useEffect(()=>{
    dispatch(fetchreplyinternalMemo({page:pageNum,sort:sortingNum,status}))
  ...
  useEffect(() => {
    const fetchReplyPaths = async () => {
      ...
      try {
        ...
        for (let intMemoLetter of internalMemoState?.replyinternalMemo?.internalMemos) {
          const response = await  apiRequest
          .get(`/rply_internal_memo_api/get_reply_memo_ltr_path/${intMemoLetter?._id}`, {
            headers: {
              Authorization: "Token " + token,
              get_rplyintmemspath_api: process.env.REACT_APP_GET_RPLYINTMEMSPATH_API,
            },
          })
          replyPathsData[intMemoLetter?._id] = response?.data?.repliedDocs;
        }
        setLoading(false);
  ...
  const handleNext = () => {
    if (pageNum >= internalMemoState?.replyinternalMemo?.totalPages) {
      setPageNum(internalMemoState?.replyinternalMemo?.totalPages);
    } else {
      setPageNum(pageNum + 1);
    }
  };

  // console.log("internalMemoState: ",internalMemoState)
  ...
  return (
    ...
    <div className="w-[95%] my-[30px] mx-auto">
      <div className="flex pb-4 justify-between items-center border-b">
        ...
        {translationState?.lan==="Am" && `${language?.repliedMemo[1]}`}</span>
  ...
  {internalMemoState?.loading ? (
      ...
      internalMemoState?.replyinternalMemo?.internalMemos?.length > 0 &&
      internalMemoState?.replyinternalMemo?.totalPages && (
        <div className="w-[100%] flex justify-end items-center my-[20px] gap-5">
          ...
          <span className="text-gray-600 font-semibold">
            {internalMemoState?.replyinternalMemo?.currentPage} of{" "}
            {internalMemoState?.replyinternalMemo?.totalPages}
          </span>
...
{(loading || internalMemoState?.loading) ? <Loading addtionalStyle={"flex justify-center items-center"}/>:internalMemoState?.replyinternalMemo?.internalMemos?.length ===0 ?   <div className="flex justify-center items-center my-[30px]">
            <span className="text-[#0C73B8] font-bold text-[18px]">
            {translationState?.lan==="En" && language?.noRepliedMemo[0]} 
            {translationState?.lan==="Am" && language?.noRepliedMemo[1]}
            </span>
   ...
   <th
                ...
                <tbody className="bg-white divide-y divide-gray-200 border [&>*:nth-child(even)]:bg-[#F9F9F9]">
                  {internalMemoState?.replyinternalMemo?.internalMemos?.map((ol,index)=>{
                    const intMemoLetterReplyPath = repliedPath[ol?._id];
                    const officeForwardedPath = intMemoLetterReplyPath?.some((ou)=>ou?.from_office_user?._id ===userID);

                    return <tr
                      key={index}
                    className="text-center border border-gray-300 text-[12px] cursor-pointer max-lg2:text-[10px]"
                    onClick={() => navigate(`/letters/memo/replied/${ol?._id}`)}
                  >
...
export default RepliedInternalMemo