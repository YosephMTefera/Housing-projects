...
import { fetchForwardInternalMemo } from '../../../REDUX/slices/forwardInternalMemoSlice';
import apiRequest from '../../../utils/request';
function ForwardedInternalMemo() {
  ...
  const internalMemoState = useSelector((state)=>state?.forwardInternalMemo);
  ...
  useEffect(()=>{
    dispatch(fetchForwardInternalMemo({page:pageNum,sort:sortingNum,status}))
  ...
  useEffect(() => {
    const fetchForwardPaths = async () => {
      ...
      try {
        ...
        for (let memoLetter of internalMemoState?.forwardInternalMemo?.internalMemos) {
          const response = await  apiRequest
          .get(`/frwd_internal_memo_api/get_forward_internal_memo_path/${memoLetter?._id}`, {
            headers: {
              Authorization: "Token " + token,
                get_frwdgetintmempath_api: process.env.REACT_APP_GET_FRWDGETINTMEMPATH_API,
            },
          })
          forwardPathsData[memoLetter?._id] = response?.data?.forwardDocs;
        }
        setLoading(false);
  ...
  const handleNext = () => {
    if (pageNum >= internalMemoState?.forwardInternalMemo?.totalPages) {
      setPageNum(internalMemoState?.forwardInternalMemo?.totalPages);
    } else {
      setPageNum(pageNum + 1);
    }
  };
    ...
    <div className="w-[95%] my-[30px] mx-auto">
      <div className="flex pb-4 justify-between items-center border-b">
        <span className="text-[#0C73B8] font-bold text-[20px] max-lg2:text-[16px]">        
          {translationState?.lan==="En" && `${language?.forwardedMemo[0]}`}
        {translationState?.lan==="Am" && `${language?.forwardedMemo[1]}`}</span>
  ...
  {internalMemoState?.loading ? (
      ...
      internalMemoState?.forwardInternalMemo?.internalMemos?.length > 0 &&
      internalMemoState?.forwardInternalMemo?.totalPages && (
        <div className="w-[100%] flex justify-end items-center my-[20px] gap-5">
          ...
          <span className="text-gray-600 font-semibold">
            {internalMemoState?.forwardInternalMemo?.currentPage} of{" "}
            {internalMemoState?.forwardInternalMemo?.totalPages}
          </span>
...
{(loading || internalMemoState?.loading) ? <Loading addtionalStyle={"flex justify-center items-center"}/>:internalMemoState?.forwardInternalMemo?.internalMemos?.length ===0 ?   <div className="flex justify-center items-center my-[30px]">
   ...
   <th
                ...
                <tbody className="bg-white divide-y divide-gray-200 border [&>*:nth-child(even)]:bg-[#F9F9F9]">
                  {internalMemoState?.forwardInternalMemo?.internalMemos?.map((ol,index)=>{
                     const intMemoLetterForwardPath = forwardPath[ol?._id];
                     const officeForwardedPath = intMemoLetterForwardPath?.some((ou)=>ou?.from_office_user?._id ===userID);

                    return <tr
                      key={index}
                    className="text-center border border-gray-300 text-[12px] cursor-pointer max-lg2:text-[10px]"
                    onClick={() => navigate(`/letters/memo/forwarded/${ol?._id}`)}
                  >
...
export default ForwardedInternalMemo