...
import { fetchreplyPresystemLetter } from "../../../REDUX/slices/repliedPresystemLettersSlice";
import apiRequest from "../../../utils/request";

function RepliedPresystemLetters() {
  ...
  const presystemLetterState = useSelector(
    (state) => state?.replyPresystemLetters
  );
  ...
  useEffect(() => {
    dispatch(
      fetchreplyPresystemLetter({
        page: pageNum,
        sort: sortingNum,
        type,
        letter_number: debouncedLetterNumber,
        status,
      })
    );
  ...
  useEffect(() => {
    const fetchReplyPaths = async () => {
      ...
      try {
        ...
        for (let preLetter of presystemLetterState?.replyPresystemLetters
          ?.letters) {
          const response = await apiRequest.get(
            `/reply_letter_api/get_reply_ltr_path/${preLetter?._id}`,
            {
              headers: {
          ...
          replyPathsData[preLetter?._id] = response?.data?.repliedDocs;
        }
        setLoading(false);
  ...
  const handleNext = () => {
    if (pageNum >= presystemLetterState?.replyPresystemLetters?.totalPages) {
      setPageNum(presystemLetterState?.replyPresystemLetters?.totalPages);
    } else {
      setPageNum(pageNum + 1);
    }
  };
      ...
      <div className="w-[95%] my-[30px] mx-auto">
        <div className="flex pb-4 justify-between items-center border-b">
          <span className="text-[#0C73B8] font-bold text-[20px] max-lg2:text-[16px]">
            {translationState?.lan === "En" &&
              `${language?.repliedLetters[0]} (${language?.presystem[0]})`}
            {translationState?.lan === "Am" &&
              `${language?.repliedLetters[1]} (${language?.presystem[1]})`}
          </span>
        </div>
        <div className="my-[20px] grid grid-cols-4 gap-[10px]">
          ...
          <div className="col-span-1 h-[90%] border border-gray-300 p-3 rounded-[5px]">
            <select
              ...
              className="w-[100%]  bg-transparent outline-none text-[14px] text-gray-500 max-lg2:text-[12px]"
            >
              <option value="">
        ...
        <div>
          {presystemLetterState?.loading ? (
            ...
            presystemLetterState?.replyPresystemLetters?.letters?.length > 0 &&
            presystemLetterState?.replyPresystemLetters?.totalPages && (
              <div className="w-[100%] flex justify-end items-center my-[20px] gap-5">
                ...
                <span className="text-gray-600 font-semibold">
                  {presystemLetterState?.replyPresystemLetters?.currentPage} of{" "}
                  {presystemLetterState?.replyPresystemLetters?.totalPages}
                </span>
        ...
        {loading || presystemLetterState?.loading ? (
          <Loading addtionalStyle={"flex justify-center items-center"} />
        ) : presystemLetterState?.replyPresystemLetters?.letters?.length ===
          0 ? (
                  ...
                  <table className="min-w-full divide-y divide-gray-200">
                    <thead className="bg-[#0C73B8] whitespace-nowrap">
                      <tr className="text-[12px] max-lg2:text-[10px]">
                        ...
                        >
                          {translationState?.lan === "En" &&
                            language?.letterType[0]}
                          {translationState?.lan === "Am" &&
                            language?.letterType[1]}
                        </th>
                        ...
                        >
                          {translationState?.lan === "En" &&
                            language?.noOfAttachment[0]}
                          {translationState?.lan === "Am" &&
                            language?.noOfAttachment[1]}
                        </th>
                        ...
                        >
                          {translationState?.lan === "En" &&
                            language?.repliedByYou[0]}
                          {translationState?.lan === "Am" &&
                            language?.repliedByYou[1]}
                        </th>
                    ...
                    <tbody className="bg-white divide-y divide-gray-200 border [&>*:nth-child(even)]:bg-[#F9F9F9]">
                      {presystemLetterState?.replyPresystemLetters?.letters?.map(
                        (pl, index) => {
                          const preLetterReplyPath = repliedPath[pl?._id];
                          const officeForwardedPath = preLetterReplyPath?.some(
                            (ou) => ou?.from_office_user?._id === userID
                          );
                          ...
                          return (
                            <tr
                              ...
                              onClick={() =>
                                navigate(
                                  `/letters/presystem/replied/${pl?._id}`
                                )
                              }
                            >
                          ...
                          {translationState?.lan === "Am" &&
                            language?.internal[1]}
                              ...
                              <td className="px-6 py-4 whitespace-nowrap border">
                                <div className="px-6 py-2 whitespace-nowrap  text-gray-500">
                                  {pl?.createdBy?.firstname}{" "}
                                  {pl?.createdBy?.middlename}{" "}
                                  {pl?.createdBy?.lastname}
                                </div>
                              </td>
...
export default RepliedPresystemLetters;
