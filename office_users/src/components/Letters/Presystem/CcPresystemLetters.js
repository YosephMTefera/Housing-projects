...
import { fetchCcPresystemLetter } from '../../../REDUX/slices/ccPresystemLetterSlice';

function CcPresystemLetters() {
  const navigate = useNavigate();
  const dispatch = useDispatch();
  const translationState = useSelector((state)=>state.translation);
  const presystemLetterState = useSelector((state)=>state?.ccPresystemLetters);
  ...
  useEffect(()=>{
    dispatch(fetchCcPresystemLetter({page:pageNum,sort:sortingNum,type,letter_number:debouncedLetterNumber,status,pathType}))

  // eslint-disable-next-line react-hooks/exhaustive-deps
  },[pageNum,sortingNum,type,debouncedLetterNumber,status,pathType])
...
if(presystemLetterState?.error) return <ServerError />
  ...
  return (
    ...
    <div className="w-[95%] my-[30px] mx-auto">
      ...
      <span className="text-[#0C73B8] font-bold text-[20px] max-lg2:text-[16px]">        
          {translationState?.lan==="En" && `${language?.ccLetters[0]} (${language?.presystem[0]})`}
        {translationState?.lan==="Am" && `${language?.ccLetters[1]} (${language?.presystem[1]})`}</span>
      ...
      <div className="my-[20px] grid grid-cols-4 gap-[10px]">
        ...
        <div className="col-span-1 h-[90%] border border-gray-300 p-3 rounded-[5px]">
          ...
          <select onChange={(e)=>setPathType(e.target.value)}  className="w-[100%]  bg-transparent outline-none text-[14px] text-gray-500 max-lg2:text-[12px]">
              <option value="">
  ...
  {presystemLetterState?.loading ? (
      ...
      presystemLetterState?.ccPresystemLetters?.letters?.length > 0 &&
      presystemLetterState?.ccPresystemLetters?.totalPages && (
        <div className="w-[100%] flex justify-end items-center my-[20px] gap-5">
          ...
          <span className="text-gray-600 font-semibold">
            {presystemLetterState?.ccPresystemLetters?.currentPage} of{" "}
            {presystemLetterState?.ccPresystemLetters?.totalPages}
          </span>
...
{presystemLetterState?.loading ? <Loading addtionalStyle={"flex justify-center items-center"}/>:presystemLetterState?.ccPresystemLetters?.letters?.length ===0 ?   <div className="flex justify-center items-center my-[30px]">
            ...
            <div className="shadow overflow-hidden border-b border-gray-200 sm:rounded-lg">
              <table className="min-w-full divide-y divide-gray-200">
                ...
                <tbody className="bg-white divide-y divide-gray-200 border [&>*:nth-child(even)]:bg-[#F9F9F9]">
                  {presystemLetterState?.ccPresystemLetters?.letters?.map((pl,index)=>{

                    return <tr
                      key={index}
                    className="text-center border border-gray-300 text-[12px] cursor-pointer max-lg2:text-[10px]"
                    onClick={() => navigate(`/letters/presystem/cc/${pl?._id}`)}
                  >
...
export default CcPresystemLetters