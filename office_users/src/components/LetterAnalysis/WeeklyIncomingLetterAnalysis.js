...
function WeeklyIncomingLetterAnalysis() {
    ...
    useEffect(() => {
        try {
          setLoading(true);
          apiRequest
            .get("/letter_analysis/weekly_incoming_letter_analysis", {
              headers: {
  ...
  return (
    ...
    <div className='"w-[100%] col-span-1 border py-4 flex flex-col justify-center items-center rounded-[10px]'>
            ...
            {translationState?.lan ==="En" && language?.weeklyIncomingLetters[0]}
            {translationState?.lan ==="Am" && language?.weeklyIncomingLetters[1]}
            </span>
...
export default WeeklyIncomingLetterAnalysis