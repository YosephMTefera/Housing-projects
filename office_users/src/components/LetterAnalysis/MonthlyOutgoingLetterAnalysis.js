...
function MonthlyOutgoingLetterAnalysis() {
    ...
    useEffect(() => {
        try {
          setLoading(true);
          apiRequest
            .get("/letter_analysis/monthly_outgoing_letter_analysis", {
              headers: {
  ...
  return (
    loading ? <Loading addtionalStyle={"col-span-2 flex justify-center items-center"} /> :
    <div className='"w-[100%] col-span-2 border py-4 flex flex-col justify-center items-center rounded-[10px]'>
            ...
            {translationState?.lan ==="En" && language?.monthlyOutgoingLetterAnalysis[0]}
            {translationState?.lan ==="Am" && language?.monthlyOutgoingLetterAnalysis[1]}
            </span>
...
export default MonthlyOutgoingLetterAnalysis