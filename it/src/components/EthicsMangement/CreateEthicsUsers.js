...
function CreateEthicsUsers() {
  ...
  const createEthicsUser = async () => {
    try {
      const ethicsData = new FormData();
      ethicsData.append("firstname", firstname);
      ethicsData.append("middlename", middlename);
      ethicsData.append("lastname", lastname);
      ethicsData.append("username", username);
      ethicsData.append("phone", phoneNumber);
      ethicsData.append("email", email);
      ethicsData.append("password", password);
      ethicsData.append("gender", gender);
      ethicsData.append("picture", profileImg);

      setLoading(true);

      await apiRequest
        .post("/accusation_acceptor_user_api/create_accusation_acceptor", ethicsData, {
          headers: {
            get_createacc_user_api: process.env.REACT_APP_GET_CREATEACC_USER_API,
        ...
        .then((res) => {
          ...
          ;
            setTimeout(() => {
              setLoading(false);
              window.location.href = "/ethicsusers";
            }, 6000);
          }
        })
        .catch((error) => {
...
export default CreateEthicsUsers